...
def insert_mysql(fileName):
    ...
    for eve_data_index in range(len(data[1:])):
        if redisDB.hexists(redis_dict_key,str(data.iloc[eve_data_index][column_name_list[0]])+'-'+str(data.iloc[eve_data_index][column_name_list[2]])+'-'+str(data.iloc[eve_data_index][column_name_list[11]])+'-'+str(data.iloc[eve_data_index][column_name_list[12]])):
            ...
            sql = 'insert into ods_aiqicha(companyName,legalRepresentative,registeredCapital,dateofIncorporation,operatingStatus,province,urbanArea,districtOrCounty,companyType,unifiedSocialCreditCode,taxpayerIdentificationNumber,industrialAndCommercialRegistrationNo,organizationCode,contactNumber,industry,address,website,mailbox,natureOfBusiness,curTime)values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            cursor.execute(sql, (
                ...
                data.iloc[eve_data_index][column_name_list[18]],insertTime
            ))
            print('数据正在插入，请稍等...')
            conn.commit()
