...
redis_dict_key = 'ods_cnjtysb_hangyegongbao'
...
def insertMysql(item):
    if redisDB.hexists(redis_dict_key, str(item['title']) + '-' + str(item['publishTime']) + '-' + str(item['html'])):
        print('已存在该数据，不作处理~~~')
    else:
        redisDB.hset(redis_dict_key,
                     str(item['title']) + '-' + str(item['publishTime']) + '-' + str(item['html']), 0)
        sql = "insert into ods_cnjtysb_hangyegongbao(title,publishTime,html,formatData,insertTime)values (%s,%s,%s,%s,%s)"
...
def getUrlList(html):
    ...
    url_list = response.xpath(
        '//div[@class="row"]/div//div[contains(@class,"list-group") and contains(@class,"tab-content")]//a/@href')
    # 获取时间
    ...
    for per_url in range(len(url_list)):
        if url_list[per_url][0] == '.':
            url = 'https://www.mot.gov.cn/fenxigongbao/hangyegongbao' + url_list[per_url][2:]
...
def getListHtml():
    for pageNum in range(0, 2):
        if pageNum == 0:
            url = 'https://www.mot.gov.cn/fenxigongbao/hangyegongbao/index.html'
        else:
            url = 'https://www.mot.gov.cn/fenxigongbao/hangyegongbao/index_{}.html'.format(pageNum)
        headers = {
            'Referer': 'https://www.mot.gov.cn/fenxigongbao/hangyegongbao/index.html',
