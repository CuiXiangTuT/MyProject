...
redis_dict_key = 'ods_cnjtysb_jingjifenxi'
...
def insertMysql(item):
    if redisDB.hexists(redis_dict_key, str(item['title']) + '-' + str(item['publishTime'])):
        ...
        sql = "insert into ods_cnjtysb_jingjifenxi(title,publishTime,html,formatData,insertTime)values (%s,%s,%s,%s,%s)"
...
def getImgUrl():
    ...
    files = {'file': ('jingjifenxi.jpg', open('./imgFile/jingjifenxi.png', 'rb'))}
...
def getDetailData(Chtml, Curls):
    ...
    if response.xpath('.//div[@id="content_main"]//p'):
        ...
        for perPIndex in range(len(pList)):
            ...
            if 'img' in partStr:
                ...
                try:
                    ...
                    image.save('./imgFile/jingjifenxi.png')
        ...
        insertMysql(item)
        print("格式化数据显示：",item['formatData'])
    elif response.xpath('.//div[@id="content_main"]/div'):
        item['formatData'] = ''.join(response.xpath('.//div[@id="content_main"]//div//text()'))
        item['html'] = Chtml
        item['insertTime'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        insertMysql(item)
...
def getUrlList(html):
    ...
    url_list = response.xpath(
        '//div[@class="row"]/div//div[contains(@class,"list-group") and contains(@class,"tab-content")]//div//a/@href')
    # 获取时间
...
# 2.获取列表页信息
def getListHtml():
    for pageNum in range(0, 9):
        if pageNum == 0:
            url = 'https://www.mot.gov.cn/fenxigongbao/jingjifenxi/index.html'
        else:
            url = 'https://www.mot.gov.cn/fenxigongbao/jingjifenxi/index_{}.html'.format(pageNum)
        headers = {
            'Referer': 'https://www.mot.gov.cn/fenxigongbao/jingjifenxi/index.html',
