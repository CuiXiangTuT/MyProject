...
def insertMysql(item):
    if redisDB.hexists(redis_dict_key,
                       item['publishTime'] + '-' + str(item['Brazil']) + '-' + str(item['Argentina']) + '-' + str(
                               item['Type_C']) + '-' + str(item['Storage'])):
        print('该数据已入库，不做处理~~~')
    else:
        redisDB.hset(redis_dict_key,
                     item['publishTime'] + '-' + str(item['Brazil']) + '-' + str(item['Argentina']) + '-' + str(
                         item['Type_C']) + '-' + str(item['Storage']), 0)
...
def getData():
    ...
    df1 = pd.DataFrame(columns=['乌拉圭', '新西兰','巴西'], index=dateList)
