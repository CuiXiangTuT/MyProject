...
class RoupinhuigongyingPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'rph_gongying'
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict, item['productName'] + '-' + item['realName']):
            # 若存在，输出数据库已经存在该数据
            print('数据库中已经存在该数据，不需要为其做任何操作...')
        else:
            # 若不存在，则将URL写入Redis数据库中
            # 把key字段的值都设为0，要设成什么都可以，因为后面对比的是字段，而不是值
            self.redis_db.hset(self.redis_data_dict, item['productName'] + '-' + item['realName'], 0)
            # 进行MySQL入库处理
            sql = "insert into rph_gongying(productName,meat_name,placeOrigin,factoryNm,realName,phone,releaseTime,buy_supply,data_source,cur_time) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
