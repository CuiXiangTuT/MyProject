...
class BaiyunpiaogongyingPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'byp_gongying'
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict,
                                 str(item['meat_kind']) + '-' + str(item['prolist_name']) + '-' + str(item['price'])):
            # 若存在，输出数据库已经存在该数据
            ...
            self.redis_db.hset(self.redis_data_dict,
                               str(item['meat_kind']) + '-' + str(item['prolist_name']) + '-' + str(item['price']), 0)
            # 进行MySQL入库处理
            sql = "insert into ods_byp_gongying(meat_kind,prolist_name,price,contact,phone,phone1,phone2,cur_time) values (%s,%s,%s,%s,%s,%s,%s,%s)"
            self.cursor.execute(sql, (item['meat_kind'],item['prolist_name'], item['price'], item['contact'], item['phone'],item['phone1'], item['phone2'],item['cur_time']))
            self.db.commit()
        return item
