...
class ZhongguonongyebushujuchaxunPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'ods_chinancnybsjcx'  # 自定义
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict,
            ...
            # 进行MySQL入库处理
            sql = "insert into ods_chinanongcunnongyebusjcx(Carea,Csource,CindexName,Ctime,product,itemName,itemType,area,Cperiod,unit,Cvalue,insertTime) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            self.cursor.execute(sql, (item['Carea'],item['Csource'],item['CindexName'],item['Ctime'],item['product'],item['itemName'],item['itemType'],item['area'],item['Cperiod'],item['unit'],item['Cvalue'],item['insertTime']))
            self.db.commit()
        return item