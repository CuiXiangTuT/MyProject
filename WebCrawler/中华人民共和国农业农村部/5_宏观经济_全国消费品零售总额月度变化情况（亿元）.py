...
# 固定资产投资完成额变化
redis_dict_key = 'ods_cnncnybqgxfplszeydbhqk'
...
def insertMysql(item):
    if redisDB.hexists(redis_dict_key,
        ...
        sql = 'insert into ods_chinanongcunnongyebuqgxfplszeydbhqk(twoThousandAndTwenty,twoThousandAndTwentyOne,Cmonth,yearOnYear,ringRatio,unit,insertTime)values (%s,%s,%s,%s,%s,%s,%s)'
        cursor.execute(sql, (item['twoThousandAndTwenty'],
                             item['twoThousandAndTwentyOne'], item['Cmonth'], item['yearOnYear'], item['ringRatio'],
                             item['unit'], item['insertTime']))
        print('正在插入数据，请稍等...')
        conn.commit()


def getData():
    url = 'http://zdscxx.moa.gov.cn:8080/nyb/qgxfplsze'
    item = {}
    headers = {
    ...
    response = \
        requests.post(url=url, headers=headers, proxies={'http': 'http://' + ip['ip'] + ':' + ip['port']}).json()[
            'result'][
            'rowDatas']
