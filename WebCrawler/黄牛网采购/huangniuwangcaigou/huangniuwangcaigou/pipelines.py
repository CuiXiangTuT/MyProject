...
class HuangniuwangcaigouPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'hnw_caigou'
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict,
                                 str(item['goodsId']) + '-' + str(item['goodsName']) + '-' + str(
                                     item['minUnitPrice']) + '-' + str(item['maxUnitPrice'])):
            # 若存在，输出数据库已经存在该数据
            ...
            self.redis_db.hset(self.redis_data_dict,
                               str(item['goodsId']) + '-' + str(item['goodsName']) + '-' + str(
                                   item['minUnitPrice']) + '-' + str(item['maxUnitPrice']), 0)
            # 进行MySQL入库处理
            sql = "insert into ods_hn_caigou(goodsId,goodsName,minUnitPrice,maxUnitPrice,totalCount,goodsUnit,endingDate,title,placeAll) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            self.cursor.execute(sql, (
                item['goodsId'], item['goodsName'], item['minUnitPrice'], item['maxUnitPrice'], item['totalCount'],
                item['goodsUnit'], item['endingDate'], item['title'], item['placeAll']))
            self.db.commit()
        return item
