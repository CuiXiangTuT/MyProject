...
redis_dict_key = 'ods_xfd_hangqingfenxi'
...
def getDetailPage(Chtml):
    ...
    try:
        pList = response.xpath('//div[@class="conter_con"]//p')
        for pIndex in range(len(pList)):
            pStr = etree.tostring(pList[pIndex]).decode("utf-8")
            if 'img' in pStr:
                originalImgUrl= pList[pIndex].xpath('.//img/@src')[0]
                ...
                # 开始生成新的图片url
                newImgUrl = getImgUrl()
                item['para'+'-'+str(pIndex)] = pStr.replace(originalImgUrl,newImgUrl)
                time.sleep(2)
            else:
                item['para' + '-' + str(pIndex)] = pList[pIndex].xpath('.//text()')
    except:
        pList = response.xpath('.//div[@class="conter_con"]//div')
        for pIndex in range(len(pList)):
            pStr = etree.tostring(pList[pIndex]).decode("utf-8")
            if 'img' in pStr:
                originalImgUrl = pList[pIndex].xpath('.//strong/img/@src')[0]
                item['para' + '-' + str(pIndex)] = originalImgUrl
                # 把该图片保存到本地
                imgContent = requests.get(url=originalImgUrl)
                # 写入到本地
                image = Image.open(BytesIO(imgContent.content))
                ...
                item['para' + '-' + str(pIndex)] = pList[pIndex].xpath('.//span/text()')
    k = []
    for value in item.values():
        if len(value) > 1:
            ...
            s = '&nbsp&nbsp&nbsp&nbsp' + ''.join(value)
        k.append(s)
    try:
        # 发表时间
