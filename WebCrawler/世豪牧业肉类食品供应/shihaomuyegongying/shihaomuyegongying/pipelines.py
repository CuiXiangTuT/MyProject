...
class ShihaomuyegongyingPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'shmy_gongying'
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict,item['title'] + '-' + str(item['contact'])):
            # 若存在，输出数据库已经存在该数据
            print('数据库中已经存在该数据,不做处理~~~')
            # print('数据库中已经存在该数据,做更新处理')
            # sql = "UPDATE ods_shmy_gongying SET companyName=%s,address=%s WHERE companyId = %s AND title = %s"
            ...
            self.redis_db.hset(self.redis_data_dict,item['title'] + '-' + str(item['contact']), 0)
            # 进行MySQL入库处理
            sql = "insert into ods_shmy_gongying(title,contact,phone,standbyPhone,email,curTime) values (%s,%s,%s,%s,%s,%s)"
            self.cursor.execute(sql, (
                item['title'], item['contact'], item['phone'], item['standbyPhone'], item['email'],
                item['curTime']))
            self.db.commit()
        return item
