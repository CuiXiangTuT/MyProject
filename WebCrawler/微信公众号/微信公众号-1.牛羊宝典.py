...
redis_dict_key = 'ods_weixin_gongzhonghao_niuyangbaodian'
...
def getImgUrl(img_name):
    ...
    files = {'file': (img_name + '.jpg', open('./imgFile/niuyangbaodian/' + img_name + '.png', 'rb'))}
...
def getDetailData(url):
    ...
    for i in sectionList:
        if 'http' in i:
            ...
            f = open('./imgFile/niuyangbaodian/' + img_name + '.png', 'wb')
...
def getListData():
    ...
    jsonTotalList = requests.post(url=url, headers=headers, data=data).json()['value']['articles']
