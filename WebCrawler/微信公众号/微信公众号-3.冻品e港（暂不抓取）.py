...
redis_dict_key = 'ods_weixin_gongzhonghao_dongpinegang'
...
def insertMysql(item):
    if redisDB.hexists(redis_dict_key, str(item['title']) + '-' + str(item['publishTime'])):
        ...
        sql = "insert into ods_weixin_gongzhonghao(title,publishTime,content,gzhSource,insertTime) values(%s,%s,%s,%s,%s)"
        cursor.execute(sql, (
            item['title'], item['publishTime'], item['content'], item['gzhSource'], item['insertTime']))
        conn.commit()
        print('数据正在插入，请稍后...')
...
def getImgUrl():
    ...
    files = {'file': ('dongpinegang.jpg', open('./imgFile/dongpinegang.png', 'rb'))}
...
def getDetailData(url):
    ...
    for i in sectionList:
        if 'http' in i:
            ...
            # 写入到本地
            f = open('./imgFile/dongpinegang.png', 'wb')
            ...
            content_list.append("&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" + i)
...
def getListData():
    ...
    data = {
        ...
        'account': 'coldeal',
        'nonce': '2b9d451ca',
        'xyz': 'da95c3e85521666e061ffce5f1f7ae0b',
