...
redis_dict_key = 'ods_weixin_gongzhonghao_deqin'
...
def getImgUrl():
    ...
    files = {'file': ('deqin.jpg', open('./imgFile/deqin.png', 'rb'))}
...
def getDetailData(url):
    ...
    # 内容
    sectionList = response.xpath('//div[@id="js_content"]/section/section')
    cokk = {}
    for sectionIndex in range(len(sectionList)):
        ...
        try:
            if 'img' in sectionStr and 'class="mp-video-player"' not in sectionStr:
                ...
                # 写入到本地
                f = open('./imgFile/deqin.png', 'wb')
                f.write(imgContent.content)
                print('正在生成新的图片链接，请稍等...')
                # 开始生成新的图片url
                newImgUrl = getImgUrl()
                print("正在生成新的图片地址...")
                newImgUrl1 = '<img src="' + newImgUrl + '">'
                cokk['para' + str(sectionIndex)] = [newImgUrl1]
                print("新的图片地址已生成...")
            else:
...
def getListData():
    ...
    data = {
        ...
        'account': 'deloitte_china',
        'nonce': 'f698720e1',
        'xyz': 'c9db039d10662a864ec72e18322b97d7',
