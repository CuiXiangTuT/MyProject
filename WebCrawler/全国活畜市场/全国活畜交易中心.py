...
def saveData(result):
    ...
    file_market = open('./全国活畜交易中心/全国活畜交易中心.txt', 'a+', encoding='utf-8')
    for eve_data in result:
        if eve_data['info'] == 'OK':
            ...
            if item['area_market_count'] == '0':
                ...
                sheet = work_book.add_sheet('全国牛羊市场')
                sheet.write(0, 0, 'id')
                sheet.write(0, 1, 'name')
                sheet.write(0, 2, 'type')
                sheet.write(0, 3, 'typecode')
                sheet.write(0, 4, 'address')
                sheet.write(0, 5, 'location')
                sheet.write(0, 6, 'tel')
                sheet.write(0, 7, 'pname')
                sheet.write(0, 8, 'cityname')
                sheet.write(0, 9, 'adname')
                sheet.write(0, 10, 'photos')
                ...
                for eve_pois in pois_list:
                    ...
                    if len(photos_list):
                        ...
                        for per_photo in eve_pois['photos']:
                            url_img = per_photo['url']
                            img_url_list.append(url_img)
                        item['photos'] = '\t'.join(img_url_list)
                        file_market.write(str(item))
                        file_market.write('\n')
                        sheet.write(row_num, 0, item['id'])
                        sheet.write(row_num, 1, item['name'])
                        sheet.write(row_num, 2, item['type'])
                        sheet.write(row_num, 3, item['typecode'])
                        sheet.write(row_num, 4, item['address'])
                        sheet.write(row_num, 5, item['location'])
                        sheet.write(row_num, 6, item['tel'])
                        sheet.write(row_num, 7, item['pname'])
                        sheet.write(row_num, 8, item['cityname'])
                        sheet.write(row_num, 9, item['adname'])
                        sheet.write(row_num, 10, item['photos'])
                        row_num += 1
                        file_name = './全国活畜交易中心/全国活畜交易中心' + '-' + str(eve_data['pois'][0]['pname']) + '-' + str(
...
def run(province_list):
    ...
    for province in province_list:
        for j in range(1, 20):
            url = 'https://restapi.amap.com/v3/place/text?key=23fa8695df583dd27453d150d2289831&keywords=%E6%B4%BB%E7%95%9C&types=&city={}&children=1&offset=50&page={}&extensions=all'
