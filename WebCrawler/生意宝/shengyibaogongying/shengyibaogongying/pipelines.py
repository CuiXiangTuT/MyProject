...
class ShengyibaogongyingPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.redis_data_dict = 'syb_gongying'
        # 链接MySQL数据库
    ...
    def process_item(self, item, spider):
        ...
        if self.redis_db.hexists(self.redis_data_dict,
                                 str(item['meatName']) + '-' + str(item['title']) + '-' + str(
                                     item['contact']) + '-' + str(
                                     item['phone']) + '-' + item['fax']):
            # 若存在，输出数据库已经存在该数据
            ...
            self.redis_db.hset(self.redis_data_dict,
                               str(item['meatName']) + '-' + str(item['title']) + '-' + str(item['contact']) + '-' + str(
                                   item['phone'])+'-'+item['fax'], 0)
            # 进行MySQL入库处理
            sql = "insert into ods_syb_gongying(meatName,title,company,price,contact,phone,mobile,fax,addr,postCodes,cur_time) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            self.cursor.execute(sql, (
                item['meatName'], item['title'], item['company'], item['price'], item['contact'],
                item['phone'],
                item['mobile'], item['fax'], item['addr'], item['postCodes'], item['cur_time']))
            self.db.commit()
        return item
