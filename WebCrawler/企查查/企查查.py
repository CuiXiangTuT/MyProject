...
def insert_mysql(fileName):
    ...
    for eve_data_index in range(len(data[1:])):
        if redisDB.hexists(redis_dict_key, str(data.iloc[eve_data_index][column_name_list[9]]) + '-' + str(
                data.iloc[eve_data_index][column_name_list[13]]) + '-' + str(
            data.iloc[eve_data_index][column_name_list[14]]) + '-' + str(
            data.iloc[eve_data_index][column_name_list[15]])):
            print('该数据已经存在于数据库，不做处理~')
        else:
            insertTime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            redisDB.hset(redis_dict_key, str(data.iloc[eve_data_index][column_name_list[9]]) + '-' + str(
                data.iloc[eve_data_index][column_name_list[13]]) + '-' + str(
                data.iloc[eve_data_index][column_name_list[14]]) + '-' + str(
                data.iloc[eve_data_index][column_name_list[15]]), 0)
            sql = 'insert into ods_qichacha(companyName,registerationStatus,legalrepresentative,registeredCapital,dateofIncorporation,approvalDate,province,city,countyAndDistrict,phone,moreCalls,email,moreEmail,unifiedSocialCreditCode,taxpayerIdentificationNum,registrationNumber,organizationCode,numberofInsuredPersons,enterpriseType,industry,nameUsedBefore,englishName,website,businessAddress,lastestAnnualReportAddress,natureofBusiness,insertTime)values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
